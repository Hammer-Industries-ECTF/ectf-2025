...
class Encoder:
    ...
    def __init__(self, secrets: bytes):
        ...
        self._secrets = {k: (urlsafe_b64decode(v[0]), urlsafe_b64decode(v[1])) for k, v in secrets_data.items()}
    ...
    def encode(self, channel: int, frame: bytes, timestamp: int) -> bytes:
        ...
        channel_cipher: AES.CbcMode = AES.new(self._secrets[str(channel)][0], AES.MODE_CBC, iv=self._secrets[str(channel)][1])
        encoded_data: bytes = channel_cipher.encrypt(frame + (b'\x00' * pad_bytes_needed))
        master_cipher: AES.CbcMode = AES.new(self._secrets["master"][0], AES.MODE_CBC, iv=self._secrets["master"][1])
        encoded_frame: bytes = master_cipher.encrypt(encoded_data + timestamp.to_bytes(8, 'little') + channel.to_bytes(4, 'little') + len(frame).to_bytes(4, 'little'))
