...
pub const SUBSCRIPTIONS_ADDRESS: usize = &SUBSCRIPTIONS as *const [Subscription; 8] as usize;

pub unsafe fn init_secrets_testing(flc: &Flc) {
...
pub fn retrieve_subscriptions() -> alloc::vec::Vec<Subscription> {
    unsafe { SUBSCRIPTIONS.to_vec() }
}

pub fn retrieve_channel_secret(channel_id: u32) -> Option<&'static Secret> {
    unsafe { SECRETS.iter().find(|x| x.secret_type == SecretType::Channel(channel_id)) }
}

pub fn retrieve_master_secret() -> &'static Secret {
    unsafe { SECRETS.iter().find(|x| x.secret_type == SecretType::Master).unwrap() }
}
